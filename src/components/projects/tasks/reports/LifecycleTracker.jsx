...
const STEPS = [
    { key: 'client_identification', label: 'Client Identific...', status: 'done' },
...
function LifecycleTracker() {
    return (
        <div className='flex flex-col gap-6 rounded-lg border border-[#0000000D] bg-white p-4'>
            <h3 className='font-semibold text-[16px]/[20px] text-[#090909]'>Lifecycle Tracker</h3>

            <div className='flex min-w-max items-start gap-4 overflow-x-auto no-scrollbar pb-1 sm:min-w-0 sm:justify-between'>
                {STEPS.map((step) => (
                    <div key={step.key} className='flex w-20 shrink-0 flex-col items-center gap-2'>
            ...
            <div className='flex items-center gap-4'>
                <span className='shrink-0 font-medium text-[14px]/[24px] text-[#1B3C4A]'>Overall Lifecycle Progress</span>
                ...
                <span className='shrink-0 font-semibold text-[14px]/[24px] text-[#1B3C4A]'>{PROGRESS_PERCENT}%</span>
            </div>
        </div>
    )
}
